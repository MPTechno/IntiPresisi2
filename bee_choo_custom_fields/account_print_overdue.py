...
class third_party_ledger(report_sxw.rml_parse, common_report_header):
    ...
    def _get_inv_nm(self, line):
        print ">>>>>>>>>>>",line
        inv_ids = self.pool['account.invoice'].search(self.cr,self.uid,[('move_id','=',line.get('move_id'))])
        inv_number = ''
        if inv_ids:
            inv_number = self.pool['account.invoice'].browse(self.cr, self.uid, inv_ids[0]).invoice_default_number
        else:
            move_obj = self.pool['account.move'].browse(self.cr, self.uid, line.get('move_id'))
            print ">>>>>>>>>>>",move_obj.ref
            in_obj = self.pool['account.invoice'].search(self.cr,self.uid,[('internal_number','=',move_obj.ref)])
            if in_obj:
                inv_number = self.pool['account.invoice'].browse(self.cr, self.uid, in_obj[0]).invoice_default_number
        return inv_number
    ...
    def lines(self, partner):
        ...
        self.cr.execute(
            "SELECT l.id, l.date, j.code, acc.code as a_code, acc.name as a_name, l.ref,m.id as move_id, m.name as move_name, l.name, l.debit, l.credit, l.amount_currency,l.currency_id, c.symbol AS currency_code " \
